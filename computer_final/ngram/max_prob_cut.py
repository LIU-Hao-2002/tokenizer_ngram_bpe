...
class MaxProbCut:
    def __init__(self):
        ...
        word_count_path = "./model/word_dict_2.model"
        word_trans_path = './model/trans_dict_2.model'
...
def main():
    cuter = MaxProbCut()
    data = load_data(data_dir=r'/root/computer_final_data/SINA_News/news2016zh_valid.json',limit=1200)
    data = preprocess(data)
    seg_sentences = []
    with joblib.Parallel(n_jobs=12) as parallel:
        seg_sentences = parallel(joblib.delayed(cuter.cut)(sentence) for sentence in data)
    with open('seg_result_valid_r2.txt', 'a', encoding='utf-8') as f:
        for sentence in seg_sentences:
            f.write(' '.join(sentence) + '\n')
    # 1200条20-40分钟跑完;1213——1351;1539 1550
main()
